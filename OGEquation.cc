...
void DMSRMatrix::setupMatrix(CompositeGrid *Kimera,
			     MatrixOperator operatorType,
			     int N_update,
			     int *update,
			     intArray flagValues[],
			     bool allNeumann,
			     ...
			     gridFunction *visc,
			     bool reset,
			     sparseSolver package,
			     gridFunction *convFieldU,
			     gridFunction *convFieldV)
{
  ...
  if (operatorType == Poisson || operatorType == Poisson_with_TD_diagonal || operatorType == Grad_Visc_Div || operatorType == Grad_Visc_Div_with_TD_diagonal || operatorType == Conv_varDiff || operatorType == Conv_varDiff_with_TD_diagonal) {
    discMolecule = 9;
  ...
  if (package == PETSc && !reset) {
    ...
    MatCreateAIJ(PETSC_COMM_WORLD, N_update, N_update, PETSC_DETERMINE, PETSC_DETERMINE, 0, nnz, 0, o_nnz,&PmatM);
  ...
  for (grid = 0; grid < Kimera->nrGrids(); grid++) {
	...
	if ((*Flag)(i, j) == 0) { // hole point
	  ...
	  insertInterpolationInfo(localrow, update, grid, -(*Flag)(i, j) - 1, i, j, Kimera, iType, package);
	  if (allNeumann) {
	    if (package == Aztec) {
	      jaM[localrow + 1]++;
...
void DMSRMatrix::insertFD(int row, int localrow, int rdim, int grid, int i, int j, MatrixOperator operatorType, int gridType, doubleSerialArray *Xr, doubleSerialArray *Xs, doubleSerialArray *Yr, doubleSerialArray *Ys, doubleSerialArray *Xrr, doubleSerialArray *Xss, doubleSerialArray *Yrr, doubleSerialArray *Yss, doubleSerialArray *sqrtOfG, doubleSerialArray *viscosity, double r_step, double s_step, sparseSolver package, doubleSerialArray *convFieldU, doubleSerialArray *convFieldV, doubleSerialArray *X, doubleSerialArray *Y, CompositeGrid *Kimera)
{
  if (operatorType == Poisson || operatorType == Poisson_with_TD_diagonal) {
...
void DMSRMatrix::insertNeumann(int row, int localrow, int side, int rdim, int i, int j, int gridType, doubleSerialArray *Xr, doubleSerialArray *Xs, doubleSerialArray *Yr, doubleSerialArray *Ys, doubleSerialArray *sqrtOfG, double r_step, double s_step, sparseSolver package)
{
  if (gridType == 1) {// cartesian grid
    if (package == Aztec) {
...
void DMSRMatrix::insertRobin(int row, int localrow, int side, int rdim, int i, int j, int gridType, doubleSerialArray *Xr, doubleSerialArray *Xs, doubleSerialArray *Yr, doubleSerialArray *Ys, doubleSerialArray *sqrtOfG, double r_step, double s_step, sparseSolver package)
{
  double RobConst = 0.1;
  if (gridType == 1) {// cartesian grid
    if (package == Aztec) {
...
void DMSRMatrix::insertPeriod(int row, int localrow, int side, int rdim, int sdim, sparseSolver package, int grid, int i, int j)
{
  if (package == Aztec) {
    jaM[localrow + 1] = jaM[localrow] + 1;
...
void DMSRMatrix::insertCorner(int row, int localrow, int rdim, int corner, sparseSolver package, int i, int j)
{
  if (package == Aztec) {
    jaM[localrow + 1] = jaM[localrow] + 3;
...
void DMSRMatrix::insertInterpolationInfo(int localrow, int *update, int grid, const int donorGrid, int i, int j, CompositeGrid *Kimera, interpolationType iType, sparseSolver package)
{
  if (iType == cubic) {
...
void OGEquation::setOperator(MatrixOperator operatorType, bool allNeumann, gridFunction *visc, bool reset, gridFunction *convFieldU, gridFunction *convFieldV)
{
  int me = Communication_Manager::My_Process_Number;
  int nrProcs = Communication_Manager::numberOfProcessors();
...
void OGEquation::destroyOperator()
{
  if (packageM == Aztec) {
    free(data_orgM);
    free(externalM);
...
double DMSRMatrix::xxCoeff(int i, int j, doubleSerialArray *xr, doubleSerialArray *xs, doubleSerialArray *yr, doubleSerialArray *ys, doubleSerialArray *xrr, doubleSerialArray *xss, doubleSerialArray *yrr, doubleSerialArray *yss, doubleSerialArray *sqrtOfG, double r_step, double s_step, const double coeff[3][3])
{
...
double DMSRMatrix::yyCoeff(int i, int j, doubleSerialArray *xr, doubleSerialArray *xs, doubleSerialArray *yr, doubleSerialArray *ys, doubleSerialArray *xrr, doubleSerialArray *xss, doubleSerialArray *yrr, doubleSerialArray *yss, doubleSerialArray *sqrtOfG, double r_step, double s_step, const double coeff[3][3])
{
...
double DMSRMatrix::xCoeff(int i, int j, doubleSerialArray *xr, doubleSerialArray *xs, doubleSerialArray *yr, doubleSerialArray *ys, doubleSerialArray *sqrtOfG, double r_step, double s_step, const double coeff[3][3])
{
  return ( ( (*ys)(i, j)*(coeff[1][2] - coeff[1][0]) / (2*r_step) - (*yr)(i, j)*(coeff[2][1] - coeff[0][1]) / (2*s_step)) / (*sqrtOfG)(i, j) );
}

double DMSRMatrix::upWindxCoeff(int i, int j, doubleSerialArray *xr, doubleSerialArray *xs, doubleSerialArray *yr, doubleSerialArray *ys, doubleSerialArray *sqrtOfG, double r_step, double s_step, const double coeff[3][3], doubleSerialArray *cF)
{
  if ((*cF)(i, j) < 0) {
...
double DMSRMatrix::yCoeff(int i, int j, doubleSerialArray *xr, doubleSerialArray *xs, doubleSerialArray *yr, doubleSerialArray *ys, doubleSerialArray *sqrtOfG, double r_step, double s_step, const double coeff[3][3])
{
  return ( -(*xs)(i, j)*(coeff[1][2] - coeff[1][0]) / (2*r_step) + (*xr)(i, j)*(coeff[2][1] - coeff[0][1]) / (2*s_step)) / (*sqrtOfG)(i, j);
}

double DMSRMatrix::upWindyCoeff(int i, int j, doubleSerialArray *xr, doubleSerialArray *xs, doubleSerialArray *yr, doubleSerialArray *ys, doubleSerialArray *sqrtOfG, double r_step, double s_step, const double coeff[3][3], doubleSerialArray *cF)
{
  if ((*cF)(i, j) < 0) {
...
double DMSRMatrix::NeumannCoeff(int i, int j, int side, doubleSerialArray *xr, doubleSerialArray *xs, doubleSerialArray *yr, doubleSerialArray *ys, doubleSerialArray *sqrtOfG, double r_step, double s_step, const double coeff[3][3])
{
...
void DMSRMatrix::getInterpolationCoefficients(CompositeGrid *Kimera, interpolationType iType)
{
...
doubleSerialArray DMSRMatrix::computeInterpolationWeights(int k, CompositeGrid *Kimera, interpolationType iType)
...
doubleSerialArray& DMSRMatrix::alpha(int i, int k, CompositeGrid *Kimera)
...
doubleSerialArray& DMSRMatrix::beta(int i, int k, CompositeGrid *Kimera)
...
doubleSerialArray& DMSRMatrix::gamma(int i, int k, CompositeGrid *Kimera)
#define rstep (Kimera->r_stepM[k])
#define r_L (Kimera->interpolationCoordinatesM[k](all, 0))
#define r (Kimera->doubleInterpolationLocationM[k](all, 0))
{
...
doubleSerialArray& DMSRMatrix::delta(int i, int k, CompositeGrid *Kimera)
#define sstep (Kimera->s_stepM[k])  
#define s_L (Kimera->interpolationCoordinatesM[k](all, 1))
#define s (Kimera->doubleInterpolationLocationM[k](all, 1))  
{
...
doubleSerialArray& DMSRMatrix::epsilon(int i, int k, CompositeGrid *Kimera)
#define rstep (Kimera->r_stepM[k])
#define r_L (Kimera->interpolationCoordinatesM[k](all, 0))
#define r (Kimera->doubleInterpolationLocationM[k](all, 0))
{
...
doubleSerialArray& DMSRMatrix::zeta(int i, int k, CompositeGrid *Kimera)
#define sstep (Kimera->s_stepM[k])  
#define s_L (Kimera->interpolationCoordinatesM[k](all, 1))
#define s (Kimera->doubleInterpolationLocationM[k](all, 1))  
{
...
void OGEquation::solve(gridFunction& RHS, CompositeGrid *Kimera, double dt, bool allNeumann, bool setBCs)
{
  int me = Communication_Manager::My_Process_Number;
  int nrProcs = Communication_Manager::numberOfProcessors();
  ...
  if (equationM == Poisson_with_TD_diagonal || equationM == Grad_Visc_Div_with_TD_diagonal || equationM == Conv_varDiff_with_TD_diagonal) {
  ...
  if (packageM == Aztec) {
    if (optionsM[AZ_precond] == AZ_dom_decomp) {
      optionsM[AZ_keep_info] = 1;
    }
    //
...
void OGEquation::setBoundaryValues(gridFunction & RHS, CompositeGrid *Kimera)
{
...
void DMSRMatrix::addToDiagonal(CompositeGrid *Kimera, const double dt, intArray flagValuesM[], int update_index[], int update[], sparseSolver package)
{
  int localrow = 0;
...
void OGEquation::writeMatrixOperatorToFile(const char* const name)
{
  int me = Communication_Manager::My_Process_Number;
  ...
  if (packageM == Aztec) {
    ...
    *stdout = ofp;
    ...
    while (tempfile.getline(line, 100)) {
      fprintf(fp, "%s\n", line);
    }
    fclose(fp);
    tempfile.close();
    ...
    PetscViewerPushFormat(v2, PETSC_VIEWER_ASCII_MATLAB);
    MatView(MatrixM.PmatM, v2);
    PetscViewerPopFormat(v2);
    PetscViewerDestroy(&v2);
  }
}

void OGEquation::setOption(OPTION option, int value)
{
  if (packageM == Aztec) {
...
void OGEquation::setParameter(OPTION parameter, double value)
{
  if (packageM == Aztec) {
...
void DMSRMatrix::getLocalToGlobal(CompositeGrid *Kimera)
{
  int count = startRow;
  for (int grid = 0; grid < Kimera->nrGrids(); grid++) {
  ...
  for (int grid = 0; grid < Kimera->nrGrids(); grid++) {
    ...
    MPI_Allgather(&myLowR, 1, MPI_INT, petscLocalRLowM[grid].getDataPointer(), 1, MPI_INT, MPI_COMM_WORLD);
    int myHiR = Kimera->ub_0(grid);
    MPI_Allgather(&myHiR, 1, MPI_INT, petscLocalRHiM[grid].getDataPointer(), 1, MPI_INT, MPI_COMM_WORLD);
    int myLowS = Kimera->lb_1(grid);
    MPI_Allgather(&myLowS, 1, MPI_INT, petscLocalSLowM[grid].getDataPointer(), 1, MPI_INT, MPI_COMM_WORLD);
    int myHiS = Kimera->ub_1(grid);
    MPI_Allgather(&myHiS, 1, MPI_INT, petscLocalSHiM[grid].getDataPointer(), 1, MPI_INT, MPI_COMM_WORLD);
    intSerialArray *myLocalToGlobal;
    myLocalToGlobal = localToGlobalM[grid].getSerialArrayPointer();
    int myStartIndex = (*myLocalToGlobal)(Kimera->lb_0(grid),Kimera->lb_1(grid));
    MPI_Allgather(&myStartIndex, 1, MPI_INT, petscStartIndexM[grid].getDataPointer(), 1, MPI_INT, MPI_COMM_WORLD);
  }
}

int DMSRMatrix::ownerProcess(int i, int j, int grid)
{
