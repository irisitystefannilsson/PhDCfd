...
hget_ ## type ( type *x, char *name, int vgroup_id ) { \
  int npairs, found=FALSE, i, status; \
...
hput_ ## type ## _array_1d( type ## _array_1d * x, char *name, int vgroup_id ){\
  const int32 rank=1; \
  int32 dims[1], start[1], edges[1], base[1], sds_id, istat, ref; \
  int n; \
 \
 \
  if (file_id == -1){ \
...
/* create the array */ \
  ...
  printf("\n\nHEY !!, I've just printed a 1d array, istat = %d\n\n", istat);  \
...
hput_ ## type ## _array_2d( type ## _array_2d * x, char *name, int vgroup_id ){\
  const int32 rank=2; \
  int32 dims[2], start[2], edges[2], base[2], stride[2], sds_id, istat, ref; \
  int32 n; \
 \
 \
  if (file_id == -1){ \
...
/* create the array */ \
  sds_id = SDcreate(sd_id, name, HDFType, rank, dims); \
  istat = SDwritedata(sds_id, start, stride, edges, (unsigned char*)(x->arrayptr)); \
/* Save array lower bounds to be compatible with Bill Henshaw */  \
  SDsetattr(sds_id, "arrayBase", DFNT_INT32, rank, base ); \
/* Insert the sds into the current Vgroup.  */  \
  ref = SDidtoref(sds_id);  \
...
hput_ ## type ## _array_3d( type ## _array_3d * x, char *name, int vgroup_id ){\
  const int32 rank=3; \
  int32 dims[3], start[3], edges[3], base[3], sds_id, istat, ref; \
  int n; \
 \
  if (file_id == -1){ \
    printf("ERROR: There is no open database file!\n"); \
...
hput_ ## type ## _array_4d( type ## _array_4d * x, char *name, int vgroup_id ){\
  const int32 rank=4; \
  int32 dims[4], start[4], edges[4], base[4], sds_id, istat, ref; \
  int n; \
 \
  if (file_id == -1){ \
    printf("ERROR: There is no open database file!\n"); \
