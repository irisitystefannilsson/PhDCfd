...
namespace ins
{
  ...
  inlet_U(const doubleArray& x, 
          ...
          int side)
  {
    // std::cout << "inlet_U: ins::grid = " << ins::grid << std::endl; 
	...
	doubleArray retB = 1.*cos(y*0) + pert*30*(pow(tt,5.)/5. + pow(tt,3.)/3. - pow(tt,4.)/2) *cos(y);
	return retB;
      } else if (t > 1. + pertTime && t <= 1 + 2.*pertTime) {
	double tt = (t - 1. - pertTime)/pertTime;
	doubleArray retC = (1. + pert)*cos(y*0) - pert*30*(pow(tt,5.)/5. + pow(tt,3.)/3. - pow(tt,4.)/2) *cos(y);
	return retC;
      } else {
             ...
             int side)
  {
    if (grid == ins::grid && side == ins::side && t < accelerationTime) {
      doubleArray retA = 30*pow(-1.+t,2.)*pow(t,2.) *cos(y*0);
      return retA;
    }
	    ...
	    int side)
  {
    if (grid == 0 && side == 3) {
      if (t <= 1.) {
	doubleArray retA = 30*(pow(t,5.)/5. + pow(t,3.)/3. - pow(t,4.)/2) *cos(y*0);
	return retA;
      }
